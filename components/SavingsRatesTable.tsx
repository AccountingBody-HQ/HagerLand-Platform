      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setTypeFilter('all')}
            ...
            style={{ background: typeFilter === 'all' ? '#1C7C4C' : '#f1f5f9', color: typeFilter === 'all' ? '#fff' : '#5B6472' }}>
            All types
          </button>
          {typeKeys.map(k => (
            <button key={k} onClick={() => setTypeFilter(typeFilter === k ? 'all' : k)}
              className="text-xs font-bold px-3 py-1.5 rounded-full transition-all"
              style={{ background: typeFilter === k ? '#1C7C4C' : '#f1f5f9', color: typeFilter === k ? '#fff' : '#5B6472' }}>
              {ACCOUNT_TYPE_LABELS[k]}
            </button>
          ))}
          <button onClick={() => setShariaFilter(v => !v)}
            className="text-xs font-bold px-3 py-1.5 rounded-full transition-all"
            style={{ background: shariaFilter ? '#92400e' : '#f1f5f9', color: shariaFilter ? '#fff' : '#5B6472' }}>
            Sharia only
          </button>
        </div>
        <div className="flex items-center gap-3">
          <input placeholder="Search bank..." value={search} onChange={e => setSearch(e.target.value)}
            className="rounded-xl text-sm text-slate-800 border border-slate-200 focus:border-green/40 focus:outline-none"
        ...
        {ranked.length === 0 ? (
          <div className="py-12 text-center">
            ...
            <button onClick={() => { setTypeFilter('all'); setShariaFilter(false); setSearch('') }}
              className="mt-3 text-xs font-bold" style={{ color: '#1C7C4C' }}>
              Clear all filters
            </button>
          </div>
        ) : ranked.map(r => (
          <div key={r.rank + r.bank + r.type}
            className={'border-b border-slate-100 transition-colors ' + (r.rank === 1 ? 'bg-green-soft' : 'bg-white hover:bg-slate-50')}>
            <div className="hidden sm:grid items-center"
              style={{ gridTemplateColumns: '44px 1fr 140px 120px 120px 110px', padding: r.rank === 1 ? '18px 24px' : '14px 24px' }}>
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black"
                style={r.rank === 1 ? { background: '#1C7C4C', color: '#fff' } : { background: '#f1f5f9', color: '#5B6472' }}>
              ...
              <p className={'font-mono font-black ' + (r.rank === 1 ? 'text-green-dark' : 'text-slate-800')} style={{ fontSize: r.rank === 1 ? '26px' : '20px', letterSpacing: '-1px' }}>{r.rate}%</p>
              <div className="flex items-center gap-1.5">
                <span style={{ color: r.freshness === 'fresh' ? '#1C7C4C' : r.freshness === 'warn' ? '#d97706' : '#ef4444' }}><ClockIcon /></span>
