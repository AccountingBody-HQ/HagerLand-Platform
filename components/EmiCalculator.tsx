      ...
      <div style={{ padding: '32px' }}>
        <div className="flex items-center gap-3 mb-6">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: 'rgba(29,78,216,0.08)' }}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#1C7C4C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
          <div>
            ...
            <input type="number" value={principal} onChange={e => setPrincipal(e.target.value)}
              className="w-full rounded-xl font-mono font-bold text-slate-900 border border-slate-200 focus:border-green focus:outline-none transition-colors"
            ...
            <div className="flex gap-1.5 mt-2 flex-wrap">
              {['100000','250000','500000','1000000'].map(v => (
                <button key={v} onClick={() => setPrincipal(v)}
                  ...
                  style={{ background: principal === v ? '#1C7C4C' : '#f1f5f9', color: principal === v ? '#fff' : '#5B6472' }}>
            ...
            <div className="flex gap-1.5 mt-2 flex-wrap">
              {['12.00','13.00','14.00','15.00'].map(v => (
                <button key={v} onClick={() => setRate(v)}
                  className="text-xs font-bold px-2 py-0.5 rounded-full transition-all"
                  style={{ background: rate === v ? '#1C7C4C' : '#f1f5f9', color: rate === v ? '#fff' : '#5B6472' }}>
                  {v}%
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-1.5 mt-2 flex-wrap">
              {[{l:'1yr',v:'12'},{l:'3yr',v:'36'},{l:'5yr',v:'60'},{l:'10yr',v:'120'}].map(o => (
                <button key={o.v} onClick={() => setMonths(o.v)}
                  className="text-xs font-bold px-2 py-0.5 rounded-full transition-all"
                  style={{ background: months === o.v ? '#1C7C4C' : '#f1f5f9', color: months === o.v ? '#fff' : '#5B6472' }}>
                  {o.l}
                </button>
              ))}
            </div>
        ...
        {emi !== null && (
          <>
            <div className="rounded-2xl p-6 mb-5" style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)', border: '1px solid #bfdbfe' }}>
              <p className="text-xs font-black uppercase tracking-widest mb-1" style={{ color: '#1C7C4C' }}>Monthly EMI</p>
              <p className="font-mono font-black" style={{ fontSize: 'clamp(32px, 5vw, 48px)', color: '#1C7C4C', letterSpacing: '-2px', lineHeight: 1 }}>
                ETB {fmt(emi)}
              </p>
              <p className="text-xs text-green mt-1">Per month for {months} months</p>
            </div>

            <div className="grid grid-cols-3 gap-3 mb-5">
              {[
                { label: 'Principal',      value: 'ETB ' + fmt(parseFloat(principal)), color: '#1C7C4C' },
                { label: 'Total Interest', value: 'ETB ' + fmt(interest!),             color: '#ef4444' },
                { label: 'Total Payment',  value: 'ETB ' + fmt(total!),                color: '#152238' },
              ].map(s => (
            ...
            <div>
              ...
              <div className="rounded-full overflow-hidden" style={{ height: 8, background: '#fee2e2' }}>
                <div className="h-full rounded-full transition-all" style={{ width: pct + '%', background: '#1C7C4C' }} />
              </div>
            </div>
          </>
        )}
