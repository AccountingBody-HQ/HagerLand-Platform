...
export default async function BusinessProfilePage({ params }: Props) {
  ...
  const { data: relatedBusinesses } = await supabase.from('money')
    .select('id, company_name, sic_description, trading_address_city, is_verified')
    .eq('status', 'active')
    .neq('id', params.id)
    .or(`sic_description.eq.${listing.category},trading_address_city.eq.${listing.city}`)
    .limit(3)
          ...
          <div className="flex items-start gap-7 mb-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <ShareButton title={listing.title} dark={true} />
                    ...
                    return (
                      <a key={b.id} href={`/listing/${b.id}`} className='flex items-center gap-4 px-6 py-4 hover:bg-section transition-colors group'>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-xs text-muted'>{b.sic_description || b.trading_address_city || 'Community listing'}</p>
                        </div>
                        {b.is_verified && (
