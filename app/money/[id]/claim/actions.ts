...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/money/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/money/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/money/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/money/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/money/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/money/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/money/${listingId}/claim?success=sent`)
}
