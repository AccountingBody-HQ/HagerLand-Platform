...
export const metadata = {
  title: 'Money transfers & financial services | HagerLand',
  description: 'Find trusted money transfer and financial services across the diaspora community.',
}

const PAGE_SIZE = 20

export default async function MoneyPage({ searchParams }: { searchParams: { type?: string; page?: string } }) {
  const serviceType = searchParams.type
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div className="max-w-3xl">
            <p className="inline-flex items-center gap-2.5 text-white/50 text-[11px] font-bold tracking-[0.18em] uppercase mb-8">
              ሃገር <span className="w-1 h-1 rounded-full bg-white/30" /> Homeland <span className="w-1 h-1 rounded-full bg-white/30" /> Money
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-xl">
              <div className="relative flex-1">
                ...
                <form action="/search" method="get">
                  <input name="q" type="text" placeholder="Search money services..." className="w-full pl-10 pr-4 py-3.5 rounded-full border-0 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-white/50 shadow-xl shadow-black/10" />
                </form>
              </div>
              <Link href="/money/post" className="inline-flex items-center justify-center gap-2 bg-white/15 hover:bg-white/25 border border-white/30 text-white font-bold rounded-full px-6 py-3.5 text-sm transition-colors whitespace-nowrap">List a service →</Link>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-24">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8">
            <div className="flex items-center gap-3">
              {serviceTypes.length > 0 && <FilterDropdown options={serviceTypes} value={serviceType} basePath="/money" paramName="type" allLabel="All services" />}
              {serviceType && <Link href="/money" className="text-xs font-semibold text-green hover:underline">Clear</Link>}
            </div>
            {count != null && <p className="text-sm text-muted ml-auto">{count.toLocaleString()} {count === 1 ? 'service' : 'services'}</p>}
          </div>
          {error && <p className="text-sm text-red-600 mb-4">Error loading services.</p>}
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {listings && listings.length > 0 ? listings.map((listing) => (
              <Link key={listing.id} href={`/money/${listing.id}`}
                ...
                <div className="flex items-start gap-4 p-5 pb-4">
                  <div className="w-11 h-11 rounded-xl bg-green-soft flex items-center justify-center shrink-0">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-green"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>
                  </div>
                ...
                <div className="flex items-center gap-4 px-5 py-3 border-t border-border bg-white mt-auto">
                  {listing.coverage && <span className="flex items-center gap-1.5 text-xs text-muted"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>{listing.coverage}</span>}
