...
const GREEN = '#1C7C4C'
export async function postMoney(formData: FormData) {
  ...
  if (!title || !contactEmail || !submitterName || !description || !phone || !category) {
    redirect('/money/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/money/post?error=invalid-email')
  const verificationToken = crypto.randomUUID()
  const manageToken = crypto.randomUUID()
  const verifyUrl = `${baseUrl}/api/money/verify-email?token=${verificationToken}`
  const { error: dbError } = await supabase.from('money').insert({
    title: title,
  ...
  if (dbError) redirect('/money/post?error=db')
  ...
  redirect('/money/post?success=verify')
}
