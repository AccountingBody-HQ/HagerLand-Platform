...
export async function postCommunity(formData: FormData) {
  ...
  if (!title || !contactEmail || !submitterName || !description || !phone || !category) {
    redirect('/community/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/community/post?error=invalid-email')
  const verificationToken = crypto.randomUUID()
  const manageToken = crypto.randomUUID()
  const verifyUrl = `${baseUrl}/api/community/verify-email?token=${verificationToken}`
  const { error: dbError } = await supabase.from('community').insert({
    name: title,
  ...
  if (dbError) redirect('/community/post?error=db')
  ...
  redirect('/community/post?success=verify')
}
