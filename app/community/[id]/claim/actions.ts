...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/community/${listingId}/claim?error=missing`)
  }

  const { data: listing } = await supabaseAdmin
    .from('community')
    .select('name, manage_token')
    .eq('id', listingId)
    .single()

  if (!listing) redirect(`/community/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/community/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/community/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/community/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/community/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/community/${listingId}/claim?success=sent`)
}
