...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/made-in-ethiopia/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/made-in-ethiopia/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/made-in-ethiopia/${listingId}/claim?error=claimed`)

  const { data: existing } = await supabaseAdmin
    .from('business_claims')
    .select('id')
    .eq('listing_id', listingId)
    .eq('section', 'made_in_ethiopia')
    .eq('claimant_email', claimantEmail)
    .eq('status', 'pending')
    .maybeSingle()

  if (existing) redirect(`/made-in-ethiopia/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/made-in-ethiopia/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/made-in-ethiopia/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/made-in-ethiopia/${listingId}/claim?success=sent`)
}
