...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { data } = await supabase.from('made_in_ethiopia')
...
export default async function MadeInEthiopiaDetailPage({ params }: Props) {
  const { data: listing, error } = await supabase.from('made_in_ethiopia').select('*')
    .eq('id', params.id).eq('status', 'active').single()
  if (error || !listing) notFound()

  const { data: related } = await supabase.from('made_in_ethiopia')
          ...
          <div className="flex flex-col sm:flex-row items-start gap-5 sm:gap-7 mb-8">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap gap-2 mb-4">
                {listing.origin_confirmed && (
                  <span className="inline-flex items-center justify-center gap-1.5 h-5 bg-gold/20 border border-gold/40 text-white text-[11px] font-bold px-4 rounded-full">
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2 sm:gap-3">
                ...
                <Link href={listing.manage_token ? "/made-in-ethiopia/edit-link" : `/made-in-ethiopia/${listing.id}/claim`}
                ...
                <Link href={listing.manage_token ? "/made-in-ethiopia/edit-link" : `/made-in-ethiopia/${listing.id}/claim?from=promotion`}
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='flex flex-wrap divide-x divide-border'>
            {[
              ...
              { label: 'Category', value: listing.category || 'Made in Ethiopia' },
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-20'>
        <div className='grid lg:grid-cols-3 gap-8 items-start min-w-0'>
          ...
          <div className='lg:col-span-2 space-y-5 min-w-0 overflow-hidden'>
            ...
            {promoActive && (
              <div className='relative overflow-hidden rounded-2xl border border-green/20' style={{background: 'linear-gradient(135deg, #f0f9f4 0%, #ffffff 100%)'}}>
                ...
                <div className='px-6 py-3.5 border-t border-green/10 flex items-center justify-between'>
                  ...
                  <a href={listing.manage_token ? '/made-in-ethiopia/edit-link' : `/made-in-ethiopia/${listing.id}/claim`} className='text-xs text-green font-semibold hover:underline'>Update this →</a>
                </div>
              </div>
            )}
            ...
            {/* Origin declaration */}
            {listing.origin_confirmed && (
              <div className='bg-gold-soft border border-gold/20 rounded-2xl overflow-hidden'>
                <div className='px-6 py-5 border-b border-gold/20 flex items-center gap-3'>
                  <div className='w-8 h-8 rounded-lg bg-gold flex items-center justify-center shrink-0'>
                    <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='white' strokeWidth='3'><polyline points='20 6 9 17 4 12'/></svg>
                  </div>
                  <div>
                    <h2 className='font-bold text-ink text-base'>Origin declaration</h2>
                    <p className='text-xs text-muted mt-0.5'>Confirmed physically made in Ethiopia</p>
                  </div>
                </div>
                <div className='px-6 py-6'>
                  <p className='text-sm leading-relaxed text-ink/80'>{listing.origin_details}</p>
                </div>
              </div>
            )}
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              ...
              <div className='p-6 grid sm:grid-cols-3 gap-3'>
                {enquiryEmail && (
                  <EnquireButton email={enquiryEmail} section='made_in_ethiopia' listingId={String(listing.id)} label='Send enquiry' subject={`Enquiry via HagerLand — ${listing.title}`} />
                )}
                {listing.phone && (
            ...
            {!listing.manage_token && (
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                ...
                <div className='px-6 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
                  ...
                  <Link href={`/made-in-ethiopia/${listing.id}/claim`} className='inline-flex items-center gap-2 bg-green hover:bg-green-dark text-white font-bold rounded-full px-6 py-2.5 text-sm transition-colors shrink-0'>
            ...
            {related && related.length > 0 && (
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                <div className='px-6 py-5 border-b border-border'>
                  <h2 className='font-bold text-ink text-base'>More Made in Ethiopia</h2>
                ...
                <div className='divide-y divide-border'>
                  {related.map((b) => {
                    ...
                    return (
                      <a key={b.id} href={`/made-in-ethiopia/${b.id}`} className='flex items-center gap-4 px-6 py-4 hover:bg-section transition-colors group'>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-xs text-muted'>{b.category || b.city || 'Made in Ethiopia'}</p>
                        </div>
                ...
                <div className='px-6 py-4 border-t border-border'>
                  <a href='/made-in-ethiopia' className='text-sm font-semibold text-green hover:underline'>View all Made in Ethiopia listings →</a>
                </div>
              </div>
            )}
            ...
            {!promoActive && (
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                ...
                <div className='px-6 py-8 text-center'>
                  ...
                  <a href={listing.manage_token ? '/made-in-ethiopia/edit-link' : `/made-in-ethiopia/${listing.id}/claim?from=promotion`} className='inline-flex items-center gap-2 bg-green hover:bg-green-dark text-white text-xs font-bold rounded-full px-4 py-2 transition-colors'>
          ...
          <div className='space-y-5 min-w-0'>
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              ...
              <div className='divide-y divide-border'>
                {[
                  { href: '/made-in-ethiopia', label: 'All Made in Ethiopia listings', sub: 'Authentic Ethiopian products' },
