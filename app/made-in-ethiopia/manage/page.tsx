...
export default function ManageMadeInEthiopiaPage() {
  ...
  useEffect(() => {
    ...
    setToken(t)
    fetch('/api/made-in-ethiopia/manage?token=' + t)
      .then(r => r.json())
      .then(({ data, error }) => {
        ...
        setOriginConfirmed(!!data.origin_confirmed)
        setOriginDetails(data.origin_details ?? '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus('saving')
    try {
      const res = await fetch('/api/made-in-ethiopia/manage', {
        method: 'POST',
      ...
      <section className='max-w-lg mx-auto px-4 sm:px-6 py-10 sm:py-16 w-full'>
        {loading ? (
          ...
          <div className='text-center py-12'>
            ...
            <p className='text-muted text-sm'>Need help? <a href='/made-in-ethiopia/edit-link' className='text-green font-medium'>Request a new edit link.</a></p>
          </div>
        ) : status === 'saved' ? (
          <div className='text-center py-12'>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col gap-4 bg-white border border-border rounded-2xl p-6 sm:p-8'>
              ...
              <label className='text-sm font-medium text-ink'>
                ...
                <select value={category} onChange={e => setCategory(e.target.value)} required className={inp + ' cursor-pointer'}>
                  ...
                  <option key='Honey & spices' value='Honey & spices'>Honey & spices</option>
                  <option key='Textiles & fabrics' value='Textiles & fabrics'>Textiles & fabrics</option>
                  <option key='Leather goods' value='Leather goods'>Leather goods</option>
                  <option key='Crafts & art' value='Crafts & art'>Crafts & art</option>
                  <option key='Food products' value='Food products'>Food products</option>
              ...
              <div className='bg-green-soft border border-green/20 rounded-xl p-4 sm:p-5 flex flex-col gap-3'>
                ...
                <label className='flex items-start gap-3 text-sm text-ink cursor-pointer'>
                  <input type='checkbox' checked={originConfirmed} onChange={e => setOriginConfirmed(e.target.checked)} required className='mt-0.5 w-4 h-4 accent-[#1C7C4C] shrink-0' />
                ...
                <label className='text-sm font-medium text-ink'>
                  ...
                  <textarea value={originDetails} onChange={e => setOriginDetails(e.target.value)} required rows={2} className={inp + ' resize-none bg-white'} placeholder='' />
                </label>
              </div>
