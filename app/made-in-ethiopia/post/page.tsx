...
import { postMadeInEthiopia } from './actions'
import { SiteNav } from '@/components/SiteNav'
import { SiteFooter } from '@/components/SiteFooter'
import { TurnstileWidget } from '@/components/TurnstileWidget'
import { MadeInEthiopiaCategorySelect } from '@/components/MadeInEthiopiaCategorySelect'
...
export default function PostMadeInEthiopiaPage({ searchParams }: { searchParams: { success?: string; error?: string } }) {
      ...
      <section className='max-w-lg mx-auto px-4 sm:px-6 py-10 sm:py-16 w-full'>
        {success ? (
          <div className='text-center py-12'>
            ...
            <p className='text-muted text-xs mt-2'>Link expires in 24 hours. Lost the link? <a href='/made-in-ethiopia/edit-link' className='text-green font-medium'>Request a new one here.</a></p>
          </div>
        ) : (
          <>
            <div className='mb-8 text-center'>
              <h1 className='text-2xl sm:text-3xl font-bold text-ink mb-2'>List a Made in Ethiopia product</h1>
            ...
            <form action={postMadeInEthiopia} className='flex flex-col gap-4 bg-white border border-border rounded-2xl p-6 sm:p-8'>
              ...
              <label className='text-sm font-medium text-ink'>
                Listing title <span className='text-ink'>*</span>
                <input name='title' required className={inp} placeholder='e.g. Yirgacheffe Single Origin Coffee' />
              </label>
              <label className='text-sm font-medium text-ink'>
              ...
              <MadeInEthiopiaCategorySelect />
              <label className='text-sm font-medium text-ink'>
                Description <span className='text-ink'>*</span>
                <textarea name='description' required rows={4} className={inp + ' resize-none'} placeholder='Tell us about this product — what makes it special...' />
              </label>

              <div className='bg-green-soft border border-green/20 rounded-xl p-4 sm:p-5 flex flex-col gap-3'>
                <p className='text-sm font-bold text-ink'>Origin declaration</p>
                <label className='flex items-start gap-3 text-sm text-ink cursor-pointer'>
                  <input type='checkbox' name='origin_confirmed' value='true' required className='mt-0.5 w-4 h-4 accent-[#1C7C4C] shrink-0' />
                  <span>I confirm this product is physically manufactured, grown, or produced in Ethiopia <span className='text-ink'>*</span></span>
                </label>
                <label className='text-sm font-medium text-ink'>
                  How/where it&apos;s made in Ethiopia <span className='text-ink'>*</span>
                  <textarea name='origin_details' required rows={2} className={inp + ' resize-none bg-white'} placeholder='e.g. Grown and processed by smallholder farmers in the Yirgacheffe region, Gedeo Zone' />
                </label>
              </div>
