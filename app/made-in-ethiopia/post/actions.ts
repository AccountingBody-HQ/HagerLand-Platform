...
export async function postMadeInEthiopia(formData: FormData) {
  ...
  if (isHoneypotFilled(formData)) redirect('/made-in-ethiopia?error=1')

  const turnstileToken = formData.get('cf-turnstile-response') as string | null
  if (!turnstileToken) redirect('/made-in-ethiopia?error=1')
  const isHuman = await verifyTurnstileToken(turnstileToken)
  if (!isHuman) redirect('/made-in-ethiopia?error=1')
  ...
  const originConfirmed = formData.get('origin_confirmed') === 'true'
  const originDetails = (formData.get('origin_details') as string ?? '').trim()

  if (!listingTitle || !contactEmail || !submitterName || !description || !phone || !category || !originConfirmed || !originDetails) {
    redirect('/made-in-ethiopia/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/made-in-ethiopia/post?error=invalid-email')

  const verificationToken = crypto.randomUUID()
  const manageToken       = crypto.randomUUID()
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/api/made-in-ethiopia/verify-email?token=${verificationToken}`

  const { error } = await supabase.from('made_in_ethiopia').insert({
    title:               listingTitle,
  ...
  if (error && error.code !== 'PGRST204') {
    console.error('Made in Ethiopia insert error:', error)
    redirect('/made-in-ethiopia/post?error=db')
  }
  ...
  redirect('/made-in-ethiopia/post?success=verify')
}
