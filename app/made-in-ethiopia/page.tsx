...
export default async function MadeInEthiopiaPage({ searchParams }: { searchParams: { type?: string; page?: string } }) {
  const listingType = searchParams.type
  const page = Math.max(1, parseInt(searchParams.page ?? '1', 10))
  const from = (page - 1) * PAGE_SIZE
  const to = from + PAGE_SIZE - 1

  let query = supabase.from('made_in_ethiopia').select('*', { count: 'exact' })
  ...
  const { data: typeRows } = await supabase.from('made_in_ethiopia').select('category')
  ...
  function pageUrl(p: number) {
    ...
    return `/made-in-ethiopia?${params.toString()}`
  }
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-xl">
              ...
              <Link href="/made-in-ethiopia/post" className="inline-flex items-center justify-center gap-2 bg-white/15 hover:bg-white/25 border border-white/30 text-white font-bold rounded-full px-6 py-3.5 text-sm transition-colors whitespace-nowrap">Post a listing →</Link>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-24">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8">
            <div className="flex items-center gap-3">
              {listingTypes.length > 0 && <FilterDropdown options={listingTypes} value={listingType} basePath="/made-in-ethiopia" paramName="type" allLabel="All types" />}
              {listingType && <Link href="/made-in-ethiopia" className="text-xs font-semibold text-green hover:underline">Clear</Link>}
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {listings && listings.length > 0 ? listings.map((listing) => (
              <Link key={listing.id} href={`/made-in-ethiopia/${listing.id}`}
                ...
                <div className="flex items-start gap-4 p-5 pb-4">
                  <div className="w-11 h-11 rounded-xl bg-green-soft flex items-center justify-center shrink-0">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-green"><path d="M20.59 13.41L13.42 20.58a2 2 0 01-2.83 0L2.59 12.58A2 2 0 012 11.17V4a2 2 0 012-2h7.17a2 2 0 011.42.59l8 8a2 2 0 010 2.82z"/><circle cx="7.5" cy="7.5" r="1.5"/></svg>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted mt-0.5 truncate">{[listing.category, listing.city].filter(Boolean).join(' · ') || 'Made in Ethiopia listing'}</p>
                  </div>
                ...
                <div className="flex items-center gap-2 px-5 pb-4">
                  ...
                  {listing.origin_confirmed && <span className="inline-flex items-center gap-1 bg-gold-soft text-gold text-xs font-bold px-2.5 py-1 rounded-full ml-auto"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5"><polyline points="20 6 9 17 4 12"/></svg>Made in Ethiopia</span>}
                </div>
                <div className="flex items-center gap-4 px-5 py-3 border-t border-border bg-white mt-auto">
                  ...
                  {listing.origin_details && <span className="text-xs text-muted truncate ml-auto">{listing.origin_details}</span>}
                </div>
              </Link>
