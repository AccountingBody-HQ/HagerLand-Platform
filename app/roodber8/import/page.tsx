  ...
  async function handleSearch() {
    ...
    try {
      ...
      if (data1.next_page_token) {
        ...
        const res2 = await fetch('/api/admin/places-search?query=' + encodeURIComponent(query) + '&pagetoken=' + btoa(data1.next_page_token), { credentials: 'include' })
        ...
        if (!data2.error && data2.results?.length > 0) {
          ...
          if (data2.next_page_token) {
            await new Promise(r => setTimeout(r, 2500))
            const res3 = await fetch('/api/admin/places-search?query=' + encodeURIComponent(query) + '&pagetoken=' + btoa(data2.next_page_token), { credentials: 'include' })
