            ...
            {unverified.map((item: Record<string, string>) => (
              <div key={item.id} style={{ padding: '13px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <a href={`/roodber8/events/${item.id}`} style={{ fontSize: 13, fontWeight: 600, color: C.text, textDecoration: 'none' }}>{item.title}</a>
          ...
          {all.length === 0 ? (
            ...
            <div key={item.id} style={{ padding: '13px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                ...
                {item.status === 'frozen' ? (
                  <>
                    <form action={restoreFrozen.bind(null, 'events', item.id)}>
