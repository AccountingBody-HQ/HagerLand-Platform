...
export default async function AdminDeliveryDetailPage({ params, searchParams }: { params: { id: string }; searchParams: { saved?: string } }) {
  noStore()
  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <Link href='/roodber8/delivery' style={{ color: C.faint, fontSize: 13, textDecoration: 'none' }}>← All Delivery</Link>
      </div>
      ...
      <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {b.status === 'pending' && (
            <>
              <form action={approveListing.bind(null, 'delivery', b.id)}>
              ...
              <form action={rejectListing.bind(null, 'delivery', b.id)}>
          ...
          {b.status === 'active' && (
            <form action={deactivateListing.bind(null, 'delivery', b.id)}>
          ...
          <form action={deleteListing.bind(null, 'delivery', b.id)}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        {/* Edit form */}
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, padding: '20px 24px' }}>
          <h2 style={{ fontSize: 12, fontWeight: 700, color: C.faint, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 16px' }}>Edit Delivery Details</h2>
          <form action={async (fd: FormData) => {
            'use server'
            const supabase2 = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SECRET_KEY!)
            await supabase2.from('delivery').update({
            ...
            redirect(`/roodber8/delivery/${b.id}?saved=1`)
          }}>
