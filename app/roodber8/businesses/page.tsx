...
import { approveListing, rejectListing } from '@/app/roodber8/review/actions'
          ...
          <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
            {pending.map((item, idx) => (
              <div key={item.id} style={{ padding: '14px 20px', borderBottom: idx < pending.length - 1 ? `1px solid ${C.border}` : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
                <div>
                  <p style={{ fontSize: 13, fontWeight: 600, color: C.text, margin: 0 }}>{item.company_name}</p>
                  <p style={{ fontSize: 11, color: C.faint, margin: '2px 0 0' }}>{item.trading_address_city ?? '—'} · {new Date(item.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                </div>
      ...
      <div>
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
          {all.length === 0 ? (
            ...
            <div key={item.id} style={{ padding: '13px 20px', borderBottom: idx < all.length - 1 ? `1px solid ${C.border}` : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <p style={{ fontSize: 13, fontWeight: 600, color: C.text, margin: 0 }}>{item.company_name}</p>
                <p style={{ fontSize: 11, color: C.faint, margin: '2px 0 0' }}>{item.trading_address_city ?? '—'} · {new Date(item.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
              </div>
