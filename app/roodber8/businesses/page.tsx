...
import { approveListing, rejectListing, deleteListing, deactivateListing, restoreFrozen, toggleFeatured } from '@/app/roodber8/review/actions'
    ...
    return (
      <div style={{ padding: '13px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          ...
          {item.status === 'frozen' ? (
            <>
              <form action={restoreFrozen.bind(null, 'companies', item.id)}>
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <form method='GET' action='/roodber8/businesses' style={{ display: 'flex', gap: 8, flex: 1, minWidth: 220 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {STATUS_FILTERS.map(([val, label]) => (
            <a
              key={label}
              href={'/roodber8/businesses?page=1' + (q ? '&q=' + encodeURIComponent(q) : '') + (val ? '&status=' + val : '')}
              style={{
      ...
      {(q || statusFilter) ? (
        <p style={{ fontSize: 12, color: C.faint, margin: '-8px 0 16px' }}>
          Showing filtered results · <a href='/roodber8/businesses' style={{ color: C.muted }}>clear filters</a>
        </p>
      ) : null}
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
          {all.length === 0
            ? <div style={{ padding: 32, textAlign: 'center', color: C.faint, fontSize: 13 }}>{(q || statusFilter) ? 'No businesses match your search.' : 'No businesses yet.'}</div>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginTop: 24 }}>
          {page > 1 && (
            <a href={'/roodber8/businesses?' + qs(page - 1)} style={{ background: C.panel, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: '8px 18px', fontSize: 13, textDecoration: 'none' }}>← Previous</a>
          )}
          <span style={{ fontSize: 13, color: C.faint }}>Page {page} of {totalPages}</span>
          {page < totalPages && (
            <a href={'/roodber8/businesses?' + qs(page + 1)} style={{ background: C.panel, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: '8px 18px', fontSize: 13, textDecoration: 'none' }}>Next →</a>
          )}
        </div>
      )}
    </div>
