...
export default async function AdminMadeInEthiopiaDetailPage({ params, searchParams }: { params: { id: string }; searchParams: { saved?: string } }) {
  noStore()
  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <Link href='/roodber8/made-in-ethiopia' style={{ color: C.faint, fontSize: 13, textDecoration: 'none' }}>← All Made in Ethiopia</Link>
      </div>
      ...
      <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, color: C.text, margin: '0 0 8px' }}>{b.title}</h1>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {b.status === 'pending' && (
            <>
              <form action={approveListing.bind(null, 'made_in_ethiopia', b.id)}>
              ...
              <form action={rejectListing.bind(null, 'made_in_ethiopia', b.id)}>
          ...
          {b.status === 'active' && (
            <form action={deactivateListing.bind(null, 'made_in_ethiopia', b.id)}>
          ...
          <form action={deleteListing.bind(null, 'made_in_ethiopia', b.id)}>
      ...
      <div style={{ background: C.goldSoft, border: `1px solid rgba(184,138,46,0.3)`, borderRadius: 14, padding: '20px 24px', marginBottom: 20 }}>
        <h2 style={{ fontSize: 12, fontWeight: 700, color: C.gold, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 12px' }}>Origin Declaration</h2>
        <p style={{ fontSize: 13, color: b.origin_confirmed ? '#4ade80' : C.danger, fontWeight: 700, margin: '0 0 8px' }}>
          {b.origin_confirmed ? '✓ Confirmed made in Ethiopia' : '✕ Not confirmed'}
        </p>
        <p style={{ fontSize: 13, color: C.text, margin: 0, lineHeight: 1.6 }}>{b.origin_details || '—'}</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, padding: '20px 24px' }}>
          ...
          {[
            ...
            { label: 'Submitted', value: b.created_at ? new Date(b.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }) : '—' },
            ...
            { label: 'Verified listing', value: false ? '✓ Yes' : 'No' },
          ].map(({ label, value }) => (
        ...
        {/* Edit form */}
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, padding: '20px 24px' }}>
          <h2 style={{ fontSize: 12, fontWeight: 700, color: C.faint, textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 16px' }}>Edit Made in Ethiopia Details</h2>
          <form action={async (fd: FormData) => {
            'use server'
            const supabase2 = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SECRET_KEY!)
            await supabase2.from('made_in_ethiopia').update({
              ...
              origin_details: (fd.get('origin_details') as string) || null,
              origin_confirmed: fd.get('origin_confirmed') === 'on',
            })
            redirect(`/roodber8/made-in-ethiopia/${b.id}?saved=1`)
          }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: C.faint, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Listing name</label>
              <input name='title' defaultValue={b.title ?? ''} style={inp} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: C.faint, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Category</label>
              <input name='category' defaultValue={b.category ?? ''} style={inp} />
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: C.faint, textTransform: 'uppercase', letterSpacing: '0.05em' }}>City</label>
              <input name='city' defaultValue={b.city ?? ''} style={inp} />
            </div>
            <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontSize: 11, fontWeight: 600, color: C.faint, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Origin details</label>
              <textarea name='origin_details' defaultValue={b.origin_details ?? ''} rows={2} style={{ ...inp, resize: 'none' }} />
            </div>
            <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              <input type='checkbox' name='origin_confirmed' defaultChecked={!!b.origin_confirmed} id='origin_confirmed' />
              <label htmlFor='origin_confirmed' style={{ fontSize: 12, color: C.muted }}>Origin confirmed (made in Ethiopia)</label>
            </div>
