  ...
  return (
    <div style={{ padding: 32, maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <Link href='/roodber8/housing' style={{ color: C.faint, fontSize: 13, textDecoration: 'none' }}>← All Housing</Link>
      </div>
      ...
      <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {b.status === 'pending' && (
            <>
              <form action={approveListing.bind(null, 'housing', b.id)}>
              ...
              <form action={rejectListing.bind(null, 'housing', b.id)}>
          ...
          {b.status === 'active' && (
            <form action={deactivateListing.bind(null, 'housing', b.id)}>
          ...
          <form action={deleteListing.bind(null, 'housing', b.id)}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 14, padding: '20px 24px' }}>
          ...
          <form action={async (fd: FormData) => {
            ...
            await supabase2.from('housing').update({
              title:         fd.get('title') as string,
              ai_description:       (fd.get('ai_description') as string) || null,
              city: fd.get('city') as string || null,
              phone: fd.get('phone') as string,
              website: fd.get('website') as string,
              category: fd.get('sic_description') as string,
            ...
            redirect(`/roodber8/housing/${b.id}?saved=1`)
          }}>
