...
export default async function AdminTutorsPage({ searchParams }: { searchParams: { page?: string; q?: string; status?: string } }) {
  noStore()
  ...
  let queryBuilder = supabase.from('tutors').select('*', { count: 'exact' })
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        <form method='GET' action='/roodber8/tutors' style={{ display: 'flex', gap: 8, flex: 1, minWidth: 220 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {STATUS_FILTERS.map(([val, label]) => (
            <a
              key={label}
              href={'/roodber8/tutors?page=1' + (q ? '&q=' + encodeURIComponent(q) : '') + (val ? '&status=' + val : '')}
              style={{
      ...
      {(q || statusFilter) ? (
        <p style={{ fontSize: 12, color: C.faint, margin: '-8px 0 16px' }}>
          Showing filtered results · <a href='/roodber8/tutors' style={{ color: C.muted }}>clear filters</a>
        </p>
      ) : null}
          ...
          {all.length === 0 ? (
            ...
            <div key={item.id} style={{ padding: '13px 20px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                ...
                <a href={`/roodber8/tutors/${item.id}`} style={{ background: 'transparent', color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: '5px 10px', fontSize: 10, fontWeight: 600, textDecoration: 'none' }}>View</a>
                {item.status === 'active' && (
                  <a href={`/tutors/${item.id}`} target='_blank' style={{ background: 'transparent', color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: '5px 10px', fontSize: 10, fontWeight: 600, textDecoration: 'none' }}>Public</a>
                )}
                {item.status === 'active' && (
                ...
                {item.status === 'frozen' ? (
                  <>
                    <form action={restoreListing.bind(null, 'tutors', item.id)}>
                    ...
                    <form action={purgeListing.bind(null, 'tutors', item.id)}>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginTop: 24 }}>
          {page > 1 && (
            <a href={'/roodber8/tutors?' + qs(page - 1)} style={{ background: C.panel, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: '8px 18px', fontSize: 13, textDecoration: 'none' }}>← Previous</a>
          )}
          <span style={{ fontSize: 13, color: C.faint }}>Page {page} of {totalPages}</span>
          {page < totalPages && (
            <a href={'/roodber8/tutors?' + qs(page + 1)} style={{ background: C.panel, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 8, padding: '8px 18px', fontSize: 13, textDecoration: 'none' }}>Next →</a>
          )}
        </div>
      )}
    </div>
