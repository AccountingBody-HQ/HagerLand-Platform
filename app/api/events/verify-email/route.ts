...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!token) return NextResponse.redirect(new URL('/events/post?error=invalid', req.url))
    const { data, error } = await supabase.from('events').select('*').eq('verification_token', token).single()
    if (error || !data) return NextResponse.redirect(new URL('/events/post?error=invalid', req.url))
    if (data.status !== 'pending_verification') return NextResponse.redirect(new URL('/events?verified=already', req.url))
    const submittedAt = new Date(data.created_at)
    const hoursSince = (Date.now() - submittedAt.getTime()) / (1000 * 60 * 60)
    if (hoursSince > 24) {
      await supabase.from('events').delete().eq('id', data.id).select()
      return NextResponse.redirect(new URL('/events/post?error=expired', req.url))
    }
    const { error: updateErr } = await supabase.from('events').update({ status: 'pending', email_verified_at: new Date().toISOString(), verification_token: null }).eq('id', data.id)
    if (updateErr && updateErr.code !== 'PGRST204') throw new Error(updateErr.message)
    const manageUrl = `${baseUrl}/events/manage?token=${data.manage_token}`
    ...
    await resend.emails.send({
      ...
      subject: `New events listing verified — ${name}`,
      html: emailWrapper(`<p style='font-size:14px;color:#152238;'>New events listing verified: <strong>${name}</strong></p>`)
    })
    return NextResponse.redirect(new URL('/events?verified=true', req.url))
  } catch (err) {
    console.error(err)
    return NextResponse.redirect(new URL('/events/post?error=server', req.url))
  }
}
