...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: listings } = await supabase.from('money').select('id, title, name, company_name, manage_token, status').eq('contact_email', email).in('status', ['active','pending'])
    ...
    const rows = listings.map((l: Record<string, string>) => {
      ...
      return `<tr><td style='padding:12px 14px;color:#152238;font-size:14px;font-weight:600;border-bottom:1px solid #e4e6e3;'>${name}</td><td style='padding:12px 14px;border-bottom:1px solid #e4e6e3;'><a href='${baseUrl}/money/manage?token=${l.manage_token}' style='color:#1C7C4C;font-weight:600;font-size:14px;text-decoration:none;'>Edit listing →</a></td></tr>`
    }).join('')
