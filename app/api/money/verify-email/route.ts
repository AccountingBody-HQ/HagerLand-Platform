...
export async function GET(req: NextRequest) {
  try {
    ...
    if (hoursSince > 24) {
      await supabase.from('money').delete().eq('id', data.id)
      return NextResponse.redirect(new URL('/money/post?error=expired', req.url))
    }
    await supabase.from('money').update({ status: 'pending', email_verified_at: new Date().toISOString(), verification_token: null }).eq('id', data.id)
