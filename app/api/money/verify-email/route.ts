...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!token) return NextResponse.redirect(new URL('/money/post?error=invalid', req.url))
    const { data, error } = await supabase.from('money').select('*').eq('verification_token', token).single()
    if (error || !data) return NextResponse.redirect(new URL('/money/post?error=invalid', req.url))
    if (data.status !== 'pending_verification') return NextResponse.redirect(new URL('/money?verified=already', req.url))
    const submittedAt = new Date(data.created_at)
    const hoursSince = (Date.now() - submittedAt.getTime()) / (1000 * 60 * 60)
    if (hoursSince > 24) {
      await supabase.from('money').delete().eq('id', data.id).select()
      return NextResponse.redirect(new URL('/money/post?error=expired', req.url))
    }
    const { error: updateErr } = await supabase.from('money').update({ status: 'pending', email_verified_at: new Date().toISOString(), verification_token: null }).eq('id', data.id)
    ...
    await resend.emails.send({
      ...
      subject: `New money listing verified — ${name}`,
      html: emailWrapper(`<p style='font-size:14px;color:#152238;'>New money listing verified: <strong>${name}</strong></p>`)
    })
