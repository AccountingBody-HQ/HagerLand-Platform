...
export async function GET(req: NextRequest) {
  try {
    ...
    if (hoursSince > 24) {
      await supabase.from('community').delete().eq('id', data.id).select()
      return NextResponse.redirect(new URL('/community/post?error=expired', req.url))
    }
    const { error: updateErr } = await supabase.from('community').update({ status: 'pending', email_verified_at: new Date().toISOString(), verification_token: null }).eq('id', data.id)
