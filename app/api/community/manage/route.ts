...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      .from('community')
      .select('id, name, category, city, country, address, phone, website, opening_hours, instagram, facebook, whatsapp, submitter_name, status, promo_text, promo_expires_at, pending_changes, ai_description')
      .eq('manage_token', token)
      .single()
    if (error || !data) return NextResponse.json({ error: 'Invalid token' }, { status: 404 })
    return NextResponse.json({ data })
  } catch (err) {
    console.error('community/manage error:', err)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existing, error: findError } = await supabase
      .from('community')
      .select('id, status, name, category, city, country, address, phone, website, opening_hours, instagram, facebook, whatsapp, submitter_name, promo_text, promo_expires_at, ai_description')
      .eq('manage_token', token)
      .single()
