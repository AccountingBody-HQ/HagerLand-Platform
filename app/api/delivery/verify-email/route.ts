...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!token) return NextResponse.redirect(new URL('/delivery/post?error=invalid', req.url))
    const { data, error } = await supabase.from('delivery').select('*').eq('verification_token', token).single()
    if (error || !data) return NextResponse.redirect(new URL('/delivery/post?error=invalid', req.url))
    if (data.status !== 'pending_verification') return NextResponse.redirect(new URL('/delivery?verified=already', req.url))
    ...
    if (hoursSince > 24) {
      ...
      return NextResponse.redirect(new URL('/delivery/post?error=expired', req.url))
    }
    const { error: updateErr } = await supabase.from('delivery').update({ status: 'pending', email_verified_at: new Date().toISOString(), verification_token: null }).eq('id', data.id)
    if (updateErr && updateErr.code !== 'PGRST204') throw new Error(updateErr.message)
    const manageUrl = `${baseUrl}/delivery/manage?token=${data.manage_token}`
    ...
    await resend.emails.send({
      ...
      subject: `New delivery listing verified — ${name}`,
      html: emailWrapper(`<p style='font-size:14px;color:#152238;'>New delivery listing verified: <strong>${name}</strong></p>`)
    })
    return NextResponse.redirect(new URL('/delivery?verified=true', req.url))
  } catch (err) {
