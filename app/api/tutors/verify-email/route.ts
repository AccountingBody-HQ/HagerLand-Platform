...
export async function GET(req: NextRequest) {
  try {
    ...
    if (hoursSince > 24) {
      await supabase.from('tutors').delete().eq('id', data.id)
      return NextResponse.redirect(new URL('/tutors/post?error=expired', req.url))
    }
    await supabase.from('tutors').update({ status: 'pending', email_verified_at: new Date().toISOString(), verification_token: null }).eq('id', data.id)
