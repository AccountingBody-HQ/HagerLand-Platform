...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!token) return NextResponse.redirect(new URL('/made-in-ethiopia/post?error=invalid', req.url))
    const { data, error } = await supabase.from('made_in_ethiopia').select('*').eq('verification_token', token).single()
    if (error || !data) return NextResponse.redirect(new URL('/made-in-ethiopia/post?error=invalid', req.url))
    if (data.status !== 'pending_verification') return NextResponse.redirect(new URL('/made-in-ethiopia?verified=already', req.url))
    const submittedAt = new Date(data.created_at)
    const hoursSince = (Date.now() - submittedAt.getTime()) / (1000 * 60 * 60)
    if (hoursSince > 24) {
      await supabase.from('made_in_ethiopia').delete().eq('id', data.id).select()
      return NextResponse.redirect(new URL('/made-in-ethiopia/post?error=expired', req.url))
    }
    const { error: updateErr } = await supabase.from('made_in_ethiopia').update({ status: 'pending', email_verified_at: new Date().toISOString(), verification_token: null }).eq('id', data.id)
    if (updateErr && updateErr.code !== 'PGRST204') throw new Error(updateErr.message)
    const manageUrl = `${baseUrl}/made-in-ethiopia/manage?token=${data.manage_token}`
    ...
    await resend.emails.send({
      ...
      subject: `New Made in Ethiopia listing verified — ${name}`,
      html: emailWrapper(`<p style='font-size:14px;color:#152238;'>New Made in Ethiopia listing verified: <strong>${name}</strong></p>`)
    })
    return NextResponse.redirect(new URL('/made-in-ethiopia?verified=true', req.url))
  } catch (err) {
    console.error('made-in-ethiopia verify error:', err)
