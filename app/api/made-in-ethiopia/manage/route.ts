...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .select('id, title, category, city, country, address, phone, website, opening_hours, instagram, facebook, whatsapp, submitter_name, status, promo_text, promo_expires_at, pending_changes, ai_description, origin_confirmed, origin_details')
      .eq('manage_token', token)
      .single()
    if (error || !data) return NextResponse.json({ error: 'Invalid token' }, { status: 404 })
    return NextResponse.json({ data })
  } catch (err) {
    console.error('made-in-ethiopia/manage error:', err)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existing, error: findError } = await supabase
      .from('made_in_ethiopia')
      .select('id, status, title, category, city, country, address, phone, website, opening_hours, instagram, facebook, whatsapp, submitter_name, promo_text, promo_expires_at, ai_description, origin_confirmed, origin_details')
      .eq('manage_token', token)
      .single()
    ...
    const newDetails = {
      ...
      promo_expires_at: body.promo_expires_at
        ...
        : null,
      origin_details: body.origin_details?.trim() ?? null,
    }
    ...
    const oldDetails = {
      ...
      origin_details: existing.origin_details ?? null,
    }
    ...
    const updatePayload: Record<string, unknown> = { origin_confirmed: !!body.origin_confirmed }
