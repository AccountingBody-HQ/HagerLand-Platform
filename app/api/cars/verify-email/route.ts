...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!token) return NextResponse.redirect(new URL('/cars/post?error=invalid', req.url))
    const { data, error } = await supabase.from('cars').select('*').eq('verification_token', token).single()
    if (error || !data) return NextResponse.redirect(new URL('/cars/post?error=invalid', req.url))
    if (data.status !== 'pending_verification') return NextResponse.redirect(new URL('/cars?verified=already', req.url))
    ...
    await resend.emails.send({
      ...
      subject: `New cars listing verified — ${name}`,
      html: emailWrapper(`<p style='font-size:14px;color:#152238;'>New cars listing verified: <strong>${name}</strong></p>`)
    })
    return NextResponse.redirect(new URL('/cars?verified=true', req.url))
  } catch (err) {
    console.error(err)
    return NextResponse.redirect(new URL('/cars/post?error=server', req.url))
  }
}
