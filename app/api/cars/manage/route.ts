...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('cars').select('*').eq('manage_token', token).single()
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { token } = body
    if (!token) return NextResponse.json({ error: 'Missing token' }, { status: 400 })
    const { data: existing, error: findError } = await supabase.from('cars').select('*').eq('manage_token', token).single()
    ...
    const { error: updateError } = await supabase.from('cars').update(updatePayload).eq('id', existing.id)
