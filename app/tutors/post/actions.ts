...
const GREEN = '#1C7C4C'
export async function postTutors(formData: FormData) {
  ...
  if (!title || !contactEmail || !submitterName || !description || !phone || !category) {
    redirect('/tutors/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/tutors/post?error=invalid-email')
  const verificationToken = crypto.randomUUID()
  const manageToken = crypto.randomUUID()
  const verifyUrl = `${baseUrl}/api/tutors/verify-email?token=${verificationToken}`
  ...
  if (dbError) redirect('/tutors/post?error=db')
  ...
  redirect('/tutors/post?success=verify')
}
