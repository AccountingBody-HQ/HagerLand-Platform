...
export default async function VerifyClaimPage({
  ...
  const listingTitle = listing?.name ?? 'your listing'
  ...
  try {
    ...
    await resend.emails.send({
      ...
      subject: `New verified claim — Tutors: ${listingTitle}`,
      html: `<p><strong>${claim.claimant_name}</strong> (${claim.claimant_email}) has verified their claim for the Tutors listing <strong>${listingTitle}</strong>.</p><p>Review and approve at <a href="https://www.hagerland.com/roodber8/claims">https://www.hagerland.com/roodber8/claims</a></p>`,
    })
  } catch (err) {
