...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/tutors/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/tutors/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/tutors/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/tutors/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/tutors/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/tutors/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/tutors/${listingId}/claim?success=sent`)
}
