...
export default async function HousingPage({ searchParams }: { searchParams: { type?: string; page?: string } }) {
  ...
  let query = supabase.from('housing').select('*', { count: 'exact' })
  ...
  const { data: typeRows } = await supabase.from('housing').select('category')
      ...
      <section className="relative overflow-hidden bg-green">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div className="max-w-3xl">
            <p className="inline-flex items-center gap-2.5 text-white/50 text-[11px] font-bold tracking-[0.18em] uppercase mb-8">ሃገር <span className="w-1 h-1 rounded-full bg-white/30" /> Homeland <span className="w-1 h-1 rounded-full bg-white/30" /> Housing</p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-xl">
              <div className="relative flex-1">
                ...
                <form action="/search" method="get">
                  <input name="q" type="text" placeholder="Search housing listings..." className="w-full pl-10 pr-4 py-3.5 rounded-full border-0 text-sm text-ink placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-white/50 shadow-xl shadow-black/10" />
                </form>
              </div>
              <Link href="/housing/post" className="inline-flex items-center justify-center gap-2 bg-white/15 hover:bg-white/25 border border-white/30 text-white font-bold rounded-full px-6 py-3.5 text-sm transition-colors whitespace-nowrap">Post a listing →</Link>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-24">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8">
            <div className="flex items-center gap-3">
              {listingTypes.length > 0 && <FilterDropdown options={listingTypes} value={listingType} basePath="/housing" paramName="type" allLabel="All types" />}
              {listingType && <Link href="/housing" className="text-xs font-semibold text-green hover:underline">Clear</Link>}
            </div>
              ...
              <Link key={listing.id} href={`/housing/${listing.id}`}
                ...
                <div className="flex items-start gap-4 p-5 pb-4">
                  <div className="w-11 h-11 rounded-xl bg-green-soft flex items-center justify-center shrink-0">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-green"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                  </div>
                ...
                <div className="flex items-center gap-4 px-5 py-3 border-t border-border bg-white mt-auto">
                  {listing.bedrooms && <span className="flex items-center gap-1.5 text-xs text-muted"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg>{listing.bedrooms} bed</span>}
