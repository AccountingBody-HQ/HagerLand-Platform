...
export async function postHousing(formData: FormData) {
  ...
  if (!title || !contactEmail || !submitterName || !description || !phone || !category) {
    redirect('/housing/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/housing/post?error=invalid-email')
  const verificationToken = crypto.randomUUID()
  const manageToken = crypto.randomUUID()
  const verifyUrl = `${baseUrl}/api/housing/verify-email?token=${verificationToken}`
  const { error: dbError } = await supabase.from('housing').insert({
    title: title,
  ...
  if (dbError) redirect('/housing/post?error=db')
  ...
  redirect('/housing/post?success=verify')
}
