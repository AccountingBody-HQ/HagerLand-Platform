...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/jobs/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/jobs/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/jobs/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/jobs/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/jobs/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/jobs/${listingId}/claim/verify?token=${token}`
  ...
  try {
    ...
    await resend.emails.send({
      ...
      to: claimantEmail,
      subject: `Verify your claim — ${listing.title}`,
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:580px;margin:0 auto;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #E4E6E3">
          ...
          <div style="padding:36px 40px">
            ...
            <p style="color:#152238;font-size:15px;margin:0 0 16px 0">Thank you for requesting to manage the listing for <strong>${listing.title}</strong> on HagerLand.</p>
  ...
  redirect(`/jobs/${listingId}/claim?success=sent`)
}
