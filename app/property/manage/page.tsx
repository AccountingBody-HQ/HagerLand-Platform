...
export default function ManageHousingPage() {
  ...
  useEffect(() => {
    ...
    setToken(t)
    fetch('/api/property/manage?token=' + t)
      .then(r => r.json())
      .then(({ data, error }) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus('saving')
    try {
      const res = await fetch('/api/property/manage', {
        method: 'POST',
      ...
      <section className='max-w-lg mx-auto px-4 sm:px-6 py-10 sm:py-16 w-full'>
        {loading ? (
          ...
          <div className='text-center py-12'>
            ...
            <p className='text-muted text-sm'>Need help? <a href='/property/edit-link' className='text-green font-medium'>Request a new edit link.</a></p>
          </div>
        ) : status === 'saved' ? (
          <div className='text-center py-12'>
