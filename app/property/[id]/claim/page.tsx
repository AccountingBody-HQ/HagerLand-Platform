    ...
    <main className="min-h-screen bg-bg flex flex-col">
      ...
      <section className="max-w-lg mx-auto px-4 sm:px-6 py-10 sm:py-16 flex-1 w-full">
        <Breadcrumb
          crumbs={[
            { href: '/', label: 'Home' },
            { href: '/property', label: 'Housing' },
            { href: `/property/${params.id}`, label: listing.title },
            { href: `/property/${params.id}/claim`, label: 'Claim listing' },
          ]}
        />
          ...
          <>
            ...
            <p className="text-muted text-center mb-8">
              {searchParams.from === 'promotion' ? (
                <>To add promotions to <span className="font-semibold text-ink">{listing.title}</span>, you first need to verify ownership.</>
              ) : (
                <>Requesting access for <span className="font-semibold text-ink">{listing.title}</span></>
              )}
            </p>
