...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/property/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/property/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/property/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/property/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/property/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/property/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/property/${listingId}/claim?success=sent`)
}
