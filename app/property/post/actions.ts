...
export async function postHousing(formData: FormData) {
  ...
  if (isHoneypotFilled(formData)) redirect('/property?error=1')

  const turnstileToken = formData.get('cf-turnstile-response') as string | null
  if (!turnstileToken) redirect('/property?error=1')
  const isHuman = await verifyTurnstileToken(turnstileToken)
  if (!isHuman) redirect('/property?error=1')
  ...
  if (!listingTitle || !contactEmail || !submitterName || !description || !phone || !category) {
    redirect('/property/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/property/post?error=invalid-email')

  const verificationToken = crypto.randomUUID()
  const manageToken       = crypto.randomUUID()
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/api/property/verify-email?token=${verificationToken}`
  ...
  if (error && error.code !== 'PGRST204') {
    ...
    redirect('/property/post?error=db')
  }
  ...
  redirect('/property/post?success=verify')
}
