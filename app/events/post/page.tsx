...
import { EventsCategorySelect } from '@/components/EventsCategorySelect'
...
export default function PostEventsPage({ searchParams }: { searchParams: { success?: string; error?: string } }) {
      ...
      <section className='max-w-lg mx-auto px-4 sm:px-6 py-10 sm:py-16 w-full'>
        {success ? (
          <div className='text-center py-12'>
            ...
            <p className='text-muted text-xs mt-2'>Link expires in 24 hours. Lost the link? <a href='/events/edit-link' className='text-green font-medium'>Request a new one here.</a></p>
          </div>
        ) : (
          <>
            <div className='mb-8 text-center'>
              <h1 className='text-2xl sm:text-3xl font-bold text-ink mb-2'>List an event</h1>
            ...
            <form action={postEvents} className='flex flex-col gap-4 bg-white border border-border rounded-2xl p-6 sm:p-8'>
              ...
              <label className='text-sm font-medium text-ink'>
                Event name <span className='text-ink'>*</span>
                <input name='title' required className={inp} placeholder='e.g. Ethiopian New Year Celebration 2026' />
              </label>
              <label className='text-sm font-medium text-ink'>
              ...
              <EventsCategorySelect value={''} onChange={() => {}} />
              <label className='text-sm font-medium text-ink'>
