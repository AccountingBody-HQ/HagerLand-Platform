...
export default async function BusinessProfilePage({ params }: Props) {
  ...
  // Related eventes — same category or city, exclude current
  const { data: relatedBusinesses } = await supabase.from('events')
    .select('id, company_name, sic_description, trading_address_city, is_verified')
    .eq('status', 'active')
    .neq('id', params.id)
    .or(`sic_description.eq.${event.category},trading_address_city.eq.${event.city}`)
    .limit(3)

  const initial = event.title.split(' ').slice(0, 2).map((w: string) => w[0]).join('').toUpperCase()
  const enquiryEmail = event.contact_email || null
  const promoActive = event.promo_text && (!event.promo_expires_at || (() => { const exp = new Date(event.promo_expires_at); exp.setHours(23,59,59,999); return exp > new Date(); })())
  const description = event.ai_description || event.category
    ? `${event.title} is a verified community event${event.city ? ` based in ${event.city}` : ''}${event.category ? `, specialising in ${event.category}` : ''}. Listed on HagerLand — the free, verified community directory.`
    : null
  ...
  return (
    <main className='min-h-screen bg-section flex flex-col'>
      ...
      <script type='application/ld+json' dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        name: event.title,
        description: event.ai_description || event.category || undefined,
        telephone: event.phone || undefined,
        url: event.website || undefined,
        address: event.city ? { '@type': 'PostalAddress', addressLocality: event.city, addressCountry: 'GB' } : undefined,
      }) }} />
          ...
          <div className="flex items-start gap-7 mb-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-2.5 mb-8">
                ...
                {event.phone && (
                  <a href={`tel:${event.phone}`} className="inline-flex items-center gap-2 text-white/65 hover:text-white text-sm transition-colors">
                ...
                {event.website && (
                  <a href={event.website.startsWith('http') ? event.website : `https://${event.website}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-white/65 hover:text-white text-sm transition-colors">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <ShareButton title={event.title} dark={true} />
                <Link href="/event/edit-link"
      ...
      <div className='bg-white border-b border-border'>
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='flex flex-wrap divide-x divide-border'>
            {[
              { label: 'Status', value: event.is_verified ? 'Verified & Active' : 'Active' },
              { label: 'Category', value: event.category || 'Community event' },
              { label: 'Location', value: event.city || 'United Kingdom' },
        ...
        <div className='grid lg:grid-cols-3 gap-8 items-start'>
          ...
          <div className='lg:col-span-2 space-y-5'>
            ...
            {promoActive && (
              ...
              <div className='relative overflow-hidden rounded-2xl border border-green/20' style={{background: 'linear-gradient(135deg, #f0f9f4 0%, #ffffff 100%)'}}>
                ...
                <div className='px-6 py-5 border-b border-green/10 flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-muted'>Latest from {event.title}</p>
                    </div>
                  </div>
                ...
                <div className='px-6 py-6'>
                  <p className='text-sm leading-relaxed text-ink whitespace-pre-line font-medium'>{event.promo_text}</p>
                </div>
                <div className='px-6 py-3.5 border-t border-green/10 flex items-center justify-between'>
                  <p className='text-xs text-muted'>Posted by the event</p>
                  <a href='/event/edit-link' className='text-xs text-green font-semibold hover:underline'>Update this →</a>
                </div>
              </div>
            )}
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              <div className='px-6 py-5 border-b border-border flex items-center justify-between'>
                <div>
                  <h2 className='font-bold text-ink text-base'>About {event.title}</h2>
              ...
              <div className='px-6 py-6'>
                <p className='text-sm leading-relaxed text-ink/80'>
                  {event.ai_description || description || `${event.title} is a verified community event listed on HagerLand — the free, verified community directory.`}
                </p>
              </div>
              {/* Trust signals strip */}
            ...
            {!promoActive && (
              <div className='border-2 border-dashed border-border rounded-2xl px-6 py-8 text-center'>
                ...
                <a href='/event/edit-link' className='inline-flex items-center gap-2 bg-green hover:bg-green-dark text-white text-xs font-bold rounded-full px-4 py-2 transition-colors'>
            ...
            {/* Contact this event — primary CTA */}
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              <div className='px-6 py-5 border-b border-border'>
                <h2 className='font-bold text-ink text-base'>Get in touch</h2>
                <p className='text-xs text-muted mt-0.5'>Contact {event.title} directly</p>
              </div>
              <div className='p-6 grid sm:grid-cols-3 gap-3'>
                {enquiryEmail && (
                  <EnquireButton email={enquiryEmail} label='Send enquiry' subject={`Enquiry via HagerLand — ${event.title}`} />
                )}
                {event.phone && (
                  <a href={`tel:${event.phone}`} className='flex items-center justify-center gap-2 bg-section hover:bg-border border border-border text-ink font-semibold rounded-xl px-4 py-3 text-sm transition-colors'>
                    <svg width='14' height='14' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2'><path d='M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.63 19.79 19.79 0 012 1.18 2 2 0 014 .02h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z'/></svg>
                    Call now
                  </a>
                )}
                {event.website && (
                  <a href={event.website.startsWith('http') ? event.website : `https://${event.website}`} target='_blank' rel='noopener noreferrer' className='flex items-center justify-center gap-2 bg-section hover:bg-border border border-border text-ink font-semibold rounded-xl px-4 py-3 text-sm transition-colors'>
            ...
            {false && (
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                <div className='px-6 py-5 border-b border-border'>
                  <h2 className='font-bold text-ink text-base'>Is this your event?</h2>
                ...
                <div className='px-6 py-6 flex items-center justify-between gap-6'>
                  ...
                  <Link href={`/event/${params.id}/claim`} className='inline-flex items-center gap-2 bg-green hover:bg-green-dark text-white font-bold rounded-full px-6 py-2.5 text-sm transition-colors shrink-0'>
              ...
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                ...
                <div className='divide-y divide-border'>
                  {relatedBusinesses.map((b) => {
                    ...
                    return (
                      <a key={b.id} href={`/event/${b.id}`} className='flex items-center gap-4 px-6 py-4 hover:bg-section transition-colors group'>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-xs text-muted'>{b.sic_description || b.trading_address_city || 'Community event'}</p>
                        </div>
                        {b.is_verified && (
                ...
                <div className='px-6 py-4 border-t border-border'>
                  <a href='/event' className='text-sm font-semibold text-green hover:underline'>View all eventes →</a>
                </div>
              </div>
            )}
          </div>
          ...
          <div className='space-y-5'>
              ...
              <div className='divide-y divide-border'>
                {event.city && (
                  <div className='flex items-center gap-3 px-5 py-3.5'>
                    ...
                    <span className='text-sm text-ink'>{event.city}</span>
                  </div>
                )}
                {event.phone && (
                  <a href={`tel:${event.phone}`} className='flex items-center gap-3 px-5 py-3.5 hover:bg-section transition-colors'>
                    ...
                    <span className='text-sm text-green font-medium hover:underline'>{event.phone}</span>
                  </a>
                )}
                {event.website && (
                  <a href={event.website.startsWith('http') ? event.website : `https://${event.website}`} target='_blank' rel='noopener noreferrer' className='flex items-center gap-3 px-5 py-3.5 hover:bg-section transition-colors'>
                    ...
                    <span className='text-sm text-green font-medium hover:underline truncate'>{event.website.replace(/^https?:\/\//, '')}</span>
                  </a>
                )}
                {event.opening_hours && (
                  <div className='flex items-center gap-3 px-5 py-3.5'>
                    ...
                    <span className='text-sm text-ink'>{event.opening_hours}</span>
                  </div>
                )}
                {(event.instagram || event.facebook || event.whatsapp) && (
                  <div className='flex items-center gap-3 px-5 py-3.5 flex-wrap'>
                    {event.instagram && (
                      <a href={event.instagram.startsWith('http') ? event.instagram : `https://instagram.com/${event.instagram.replace('@','')}`} target='_blank' rel='noopener noreferrer' className='text-xs font-semibold text-green hover:underline'>Instagram</a>
                    )}
                    {event.facebook && (
                      <a href={event.facebook.startsWith('http') ? event.facebook : `https://${event.facebook}`} target='_blank' rel='noopener noreferrer' className='text-xs font-semibold text-green hover:underline'>Facebook</a>
                    )}
                    {event.whatsapp && (
                      <a href={`https://wa.me/${event.whatsapp.replace(/[^0-9+]/g,'')}`} target='_blank' rel='noopener noreferrer' className='text-xs font-semibold text-green hover:underline'>WhatsApp</a>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              ...
              <div className='divide-y divide-border'>
                {[
                  { href: '/event', label: 'All eventes', sub: 'Community directory' },
            ...
            <div className='bg-green rounded-2xl p-5'>
              ...
              <p className='text-base font-bold text-white mb-3'>List your event</p>
              <p className='text-xs text-white/70 mb-4'>Join the community directory — free for everyone, always.</p>
              <Link href='/event/post' className='block text-center bg-white text-green font-bold text-sm rounded-full px-4 py-2.5 hover:bg-green-soft transition-colors'>
                Get listed — free
              </Link>
            </div>
          </div>
