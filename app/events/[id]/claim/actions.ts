...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/events/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/events/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/events/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/events/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/events/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/events/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/events/${listingId}/claim?success=sent`)
}
