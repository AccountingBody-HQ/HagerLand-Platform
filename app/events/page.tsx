...
export const metadata = {
  title: 'Events — HagerLand',
  description: 'Major Ethiopian business, financial, and cultural events across Ethiopia and the diaspora.',
}
      ...
      <section className="relative overflow-hidden bg-green">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div className="max-w-3xl">
            <p className="inline-flex items-center gap-2.5 text-white/50 text-[11px] font-bold tracking-[0.18em] uppercase mb-8">ሃገር <span className="w-1 h-1 rounded-full bg-white/30" translate="no" /> Homeland <span className="w-1 h-1 rounded-full bg-white/30" translate="no" /> Events</p>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.05] tracking-tight mb-6">The Ethiopian Business Calendar</h1>
            <p className="text-white/65 text-lg sm:text-xl max-w-xl mb-10 leading-relaxed">Trade missions, investment summits, export expos, and major commercial and cultural events — the moments when the Ethiopian economy moves.</p>
            <div className="flex flex-wrap items-center gap-x-8 gap-y-4 mb-10">
              {count != null && count > 0 && <div className="flex items-baseline gap-2"><span className="text-2xl font-bold text-white" translate="no">{count.toLocaleString()}</span><span className="text-white/40 text-sm">events</span></div>}
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-xl">
              ...
              <Link href="/events/post" className="inline-flex items-center justify-center gap-2 bg-white/15 hover:bg-white/25 border border-white/30 text-white font-bold rounded-full px-6 py-3.5 text-sm transition-colors whitespace-nowrap">Post an event →</Link>
            </div>
          </div>
        </div>
      </section>
