...
export default async function TickerPage({ params }: { params: { ticker: string } }) {
  ...
  const heroStats = [
    ...
    { value: security.price_change_pct != null ? (isPositive ? '+' : '') + fmt(security.price_change_pct) + '%' : '\u2014', label: 'Change', color: security.price_change_pct != null ? (isPositive ? '#4ade80' : '#f87171') : '#5B6472' },
    ...
    <main className="bg-white flex-1">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8 mb-10">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                <span className="font-mono font-black text-sm px-3 py-1.5 rounded-lg" style={{ background: '#1C7C4C', color: '#fff' }}>
                  {security.ticker}
                </span>
                {security.sector && (
                ...
                {[
                  ...
                  <div key={i} className="flex items-start gap-3">
                    <span className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 text-xs font-black mt-0.5"
                      style={{ background: '#E9F5EE', color: '#1C7C4C' }}>{i + 1}</span>
                    <p className="text-xs text-slate-600" style={{ lineHeight: 1.7 }}>{step}</p>
                  </div>
                ))}
                <Link href="/birrbank/markets/how-to-invest"
                  className="flex items-center gap-1 text-xs font-bold mt-2" style={{ color: '#1C7C4C' }}>
      ...
      <section style={{ background: '#1C7C4C', padding: '72px 0', borderTop: '1px solid #155F3A' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-8">
          <div>
            <p className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: '#ffffff' }}>ESX Markets</p>
