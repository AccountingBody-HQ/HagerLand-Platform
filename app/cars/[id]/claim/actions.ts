...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/cars/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/cars/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/cars/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/cars/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/cars/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/cars/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/cars/${listingId}/claim?success=sent`)
}
