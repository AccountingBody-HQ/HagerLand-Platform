...
export async function postCars(formData: FormData) {
  ...
  if (isHoneypotFilled(formData)) redirect('/cars?error=1')

  const turnstileToken = formData.get('cf-turnstile-response') as string | null
  if (!turnstileToken) redirect('/cars?error=1')
  const isHuman = await verifyTurnstileToken(turnstileToken)
  if (!isHuman) redirect('/cars?error=1')
  ...
  if (!listingTitle || !contactEmail || !submitterName || !description || !phone || !category) {
    redirect('/cars/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/cars/post?error=invalid-email')

  const verificationToken = crypto.randomUUID()
  const manageToken       = crypto.randomUUID()
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/api/cars/verify-email?token=${verificationToken}`
  ...
  redirect('/cars/post?success=verify')
}
