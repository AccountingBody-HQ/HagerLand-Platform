...
export default async function VerifyClaimPage({
  ...
  const { data: claim } = await supabaseAdmin
    ...
    .select('id, claimant_name, claimant_email, code_expires_at, status, companies(company_name)')
  ...
  const companiesData = claim.companies
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: `<p><strong>${claim.claimant_name}</strong> (${claim.claimant_email}) has verified their claim for <strong>${companyName}</strong>.</p><p>Review and approve at <a href="https://hagerland.com/roodber8/review">https://hagerland.com/roodber8/review</a></p>`,
    })
  } catch (err) {
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-20">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted leading-relaxed mb-6">
            Your claim for{' '}
            <span className="font-semibold text-ink">{companyName}</span> has been verified.
