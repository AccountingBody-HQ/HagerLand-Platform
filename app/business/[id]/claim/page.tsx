...
export const metadata = { title: 'Claim your business listing' }
...
export default async function ClaimPage({
  ...
  const { data: business } = await supabase
    .from('companies')
    .select('id, company_name, is_claimed, is_verified')
    .eq('id', params.id)
    .eq('status', 'active')
    .single()
      ...
      <section className="max-w-lg mx-auto px-4 sm:px-6 py-10 sm:py-16 flex-1 w-full">
        <Breadcrumb
          crumbs={[
            ...
            { href: `/business/${params.id}/claim`, label: 'Claim listing' },
          ]}
        />
        {business.is_verified ? (
          <div className="text-center py-12">
            <p className="text-2xl font-bold text-ink mb-4">Already verified</p>
            <p className="text-muted">This listing has already been claimed and verified.</p>
          </div>
            ...
            {searchParams.error && (
              <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-4 py-3 mb-6">
                {searchParams.error === 'duplicate'
                  ? 'A claim is already pending for this business with that email.'
