...
export default async function BusinessProfilePage({ params }: Props) {
  ...
  // Related businesses — same category or city, exclude current
  const { data: relatedBusinesses } = await supabase.from('companies')
    .select('id, company_name, sic_description, trading_address_city, is_verified')
    .eq('status', 'active')
    .neq('id', params.id)
    .or(`sic_description.eq.${business.sic_description},trading_address_city.eq.${business.trading_address_city}`)
    .limit(3)
        ...
        <div className='grid lg:grid-cols-3 gap-8 items-start min-w-0'>
          ...
          <div className='lg:col-span-2 space-y-5 min-w-0 overflow-hidden'>
            ...
            {promoActive && (
              ...
              <div className='relative overflow-hidden rounded-2xl border border-green/20' style={{background: 'linear-gradient(135deg, #f0f9f4 0%, #ffffff 100%)'}}>
                ...
                <div className='px-6 py-5 border-b border-green/10 flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      ...
                      <p className='text-xs text-muted'>Latest from {business.company_name}</p>
                    </div>
                  </div>
                ...
                <div className='px-6 py-6'>
                  <p className='text-sm leading-relaxed text-ink whitespace-pre-line font-medium'>{business.promo_text}</p>
                </div>
                <div className='px-6 py-3.5 border-t border-green/10 flex items-center justify-between'>
                  <p className='text-xs text-muted'>Posted by the business</p>
                  <a href='/business/edit-link' className='text-xs text-green font-semibold hover:underline'>Update this →</a>
                </div>
              </div>
            )}
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              ...
              <div className='p-6 grid sm:grid-cols-3 gap-3'>
                ...
                {business.website && (
                  <a href={business.website.startsWith('http') ? business.website : `https://${business.website}`} target='_blank' rel='noopener noreferrer' className='flex items-center justify-center gap-2 bg-section hover:bg-border border border-border text-ink font-semibold rounded-xl px-4 py-3 text-sm transition-colors'>
              ...
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                ...
                <div className='divide-y divide-border'>
                  {relatedBusinesses.map((b) => {
                    ...
                    return (
                      <a key={b.id} href={`/business/${b.id}`} className='flex items-center gap-4 px-6 py-4 hover:bg-section transition-colors group'>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-xs text-muted'>{b.sic_description || b.trading_address_city || 'Community business'}</p>
                        </div>
                        {b.is_verified && (
          ...
          <div className='space-y-5 min-w-0'>
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              ...
              <div className='divide-y divide-border'>
                {(business.trading_address_city || business.address || business.country) && (
                  <div className='flex items-start gap-3 px-5 py-3.5'>
                    ...
                    <div className='flex flex-col gap-0.5 min-w-0'>
                      {business.address
                        ? <span className='text-sm text-ink break-words'>{business.address}</span>
                        : <>
                            {business.trading_address_city && <span className='text-sm text-ink break-words'>{business.trading_address_city}</span>}
                            {business.country && <span className='text-sm text-ink break-words'>{business.country}</span>}
                          </>
                      }
                    </div>
                  </div>
                ...
                {business.phone && (
                  <a href={`tel:${business.phone}`} className='flex items-center gap-3 px-5 py-3.5 hover:bg-section transition-colors'>
                    ...
                    <span className='text-sm text-green font-medium hover:underline'>{business.phone}</span>
                  </a>
                )}
                {business.website && (
                  <a href={business.website.startsWith('http') ? business.website : `https://${business.website}`} target='_blank' rel='noopener noreferrer' className='flex items-center gap-3 px-5 py-3.5 hover:bg-section transition-colors'>
            ...
            <div className='bg-green rounded-2xl p-5'>
              ...
              <Link href='/business/post' className='block text-center bg-white text-green font-bold text-sm rounded-full px-4 py-2.5 hover:bg-green-soft transition-colors leading-tight min-h-[38px] flex items-center justify-center'>
                Get listed — free
              </Link>
            </div>
          </div>
