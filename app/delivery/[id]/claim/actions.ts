...
export async function submitClaim(listingId: string, formData: FormData) {
  ...
  if (!claimantName || !claimantEmail) {
    redirect(`/delivery/${listingId}/claim?error=missing`)
  }
  ...
  if (!listing) redirect(`/delivery/${listingId}/claim?error=notfound`)
  if (listing.manage_token) redirect(`/delivery/${listingId}/claim?error=claimed`)
  ...
  if (existing) redirect(`/delivery/${listingId}/claim?error=duplicate`)
  ...
  if (insertError) {
    ...
    redirect(`/delivery/${listingId}/claim?error=server`)
  }

  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/delivery/${listingId}/claim/verify?token=${token}`
  ...
  redirect(`/delivery/${listingId}/claim?success=sent`)
}
