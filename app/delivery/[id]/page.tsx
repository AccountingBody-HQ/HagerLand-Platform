...
export default async function DeliveryDetailPage({ params }: Props) {
  const { data: listing, error } = await supabase.from('delivery').select('*')
    .eq('id', params.id).eq('status', 'active').single()
  if (error || !listing) notFound()

  const { data: related } = await supabase.from('delivery')
          ...
          <div className="flex flex-col sm:flex-row items-start gap-5 sm:gap-7 mb-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2 sm:gap-3">
                ...
                <Link href={listing.manage_token ? "/delivery/edit-link" : `/delivery/${listing.id}/claim`}
                ...
                <Link href={listing.manage_token ? "/delivery/edit-link" : `/delivery/${listing.id}/claim?from=promotion`}
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
          <div className='flex flex-wrap divide-x divide-border'>
            {[
              ...
              { label: 'Category', value: listing.category || 'Delivery & courier' },
        ...
        <div className='grid lg:grid-cols-3 gap-8 items-start min-w-0'>
          ...
          <div className='lg:col-span-2 space-y-5 min-w-0 overflow-hidden'>
            ...
            {promoActive && (
              <div className='relative overflow-hidden rounded-2xl border border-green/20' style={{background: 'linear-gradient(135deg, #f0f9f4 0%, #ffffff 100%)'}}>
                ...
                <div className='px-6 py-3.5 border-t border-green/10 flex items-center justify-between'>
                  ...
                  <a href={listing.manage_token ? '/delivery/edit-link' : `/delivery/${listing.id}/claim`} className='text-xs text-green font-semibold hover:underline'>Update this →</a>
                </div>
              </div>
            )}
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              ...
              <div className='p-6 grid sm:grid-cols-3 gap-3'>
                {enquiryEmail && (
                  <EnquireButton email={enquiryEmail} section='delivery' listingId={String(listing.id)} label='Send enquiry' subject={`Enquiry via HagerLand — ${listing.title}`} />
                )}
                {listing.phone && (
            ...
            {!listing.manage_token && (
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                ...
                <div className='px-6 py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
                  ...
                  <Link href={`/delivery/${listing.id}/claim`} className='inline-flex items-center gap-2 bg-green hover:bg-green-dark text-white font-bold rounded-full px-6 py-2.5 text-sm transition-colors shrink-0'>
              ...
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                ...
                <div className='divide-y divide-border'>
                  {related.map((b) => {
                    ...
                    return (
                      <a key={b.id} href={`/delivery/${b.id}`} className='flex items-center gap-4 px-6 py-4 hover:bg-section transition-colors group'>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <p className='text-xs text-muted'>{b.category || b.city || 'Delivery & courier'}</p>
                        </div>
                ...
                <div className='px-6 py-4 border-t border-border'>
                  <a href='/delivery' className='text-sm font-semibold text-green hover:underline'>View all delivery listings →</a>
                </div>
              </div>
            )}
            ...
            {!promoActive && (
              <div className='bg-white border border-border rounded-2xl overflow-hidden'>
                ...
                <div className='px-6 py-8 text-center'>
                  ...
                  <a href={listing.manage_token ? '/delivery/edit-link' : `/delivery/${listing.id}/claim?from=promotion`} className='inline-flex items-center gap-2 bg-green hover:bg-green-dark text-white text-xs font-bold rounded-full px-4 py-2 transition-colors'>
          ...
          <div className='space-y-5 min-w-0'>
            ...
            <div className='bg-white border border-border rounded-2xl overflow-hidden'>
              ...
              <div className='divide-y divide-border'>
                {[
                  { href: '/delivery', label: 'All delivery listings', sub: 'Delivery & courier' },
            ...
            <div className='bg-green rounded-2xl p-5'>
              ...
              <p className='text-base font-bold text-white mb-3'>List your delivery service</p>
              <p className='text-xs text-white/70 mb-4'>Join Ethiopia&apos;s business and financial platform — free for everyone, always.</p>
              <Link href='/delivery/post' className='block text-center bg-white text-green font-bold text-sm rounded-full px-4 py-2.5 hover:bg-green-soft transition-colors' translate='no'>
                Get listed — free
              </Link>
            </div>
          </div>
