...
export default function ManageDeliveryPage() {
  ...
  useEffect(() => {
    ...
    setToken(t)
    fetch('/api/delivery/manage?token=' + t)
      .then(r => r.json())
      .then(({ data, error }) => {
      ...
      <section className='max-w-lg mx-auto px-4 sm:px-6 py-10 sm:py-16 w-full'>
        {loading ? (
          ...
          <div className='text-center py-12'>
            ...
            <p className='text-muted text-sm'>Need help? <a href='/delivery/edit-link' className='text-green font-medium'>Request a new edit link.</a></p>
          </div>
        ) : status === 'saved' ? (
          <div className='text-center py-12'>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col gap-4 bg-white border border-border rounded-2xl p-6 sm:p-8'>
              ...
              <label className='text-sm font-medium text-ink'>
                ...
                <select value={category} onChange={e => setCategory(e.target.value)} required className={inp + ' cursor-pointer'}>
                  ...
                  <option key='Courier services' value='Courier services'>Courier services</option>
                  <option key='Same-day/local delivery' value='Same-day/local delivery'>Same-day/local delivery</option>
                  <option key='Removals & man-with-van' value='Removals & man-with-van'>Removals & man-with-van</option>
                  <option key='Freight & haulage' value='Freight & haulage'>Freight & haulage</option>
                  <option key='Cargo shipping' value='Cargo shipping'>Cargo shipping</option>
                  <option key='Logistics & warehousing' value='Logistics & warehousing'>Logistics & warehousing</option>
                  <option key='Bike/motorcycle courier' value='Bike/motorcycle courier'>Bike/motorcycle courier</option>
                  <option key='International shipping' value='International shipping'>International shipping</option>
                </select>
              </label>
              <label className='text-sm font-medium text-ink'>
