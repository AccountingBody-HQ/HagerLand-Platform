...
export async function postDelivery(formData: FormData) {
  ...
  if (isHoneypotFilled(formData)) redirect('/delivery?error=1')

  const turnstileToken = formData.get('cf-turnstile-response') as string | null
  if (!turnstileToken) redirect('/delivery?error=1')
  const isHuman = await verifyTurnstileToken(turnstileToken)
  if (!isHuman) redirect('/delivery?error=1')
  ...
  if (!listingTitle || !contactEmail || !submitterName || !description || !phone || !category) {
    redirect('/delivery/post?error=missing')
  }
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  if (!emailRegex.test(contactEmail)) redirect('/delivery/post?error=invalid-email')

  const verificationToken = crypto.randomUUID()
  const manageToken       = crypto.randomUUID()
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://hagerland-platform.vercel.app'
  const verifyUrl = `${baseUrl}/api/delivery/verify-email?token=${verificationToken}`
  ...
  if (error && error.code !== 'PGRST204') {
    ...
    redirect('/delivery/post?error=db')
  }
  ...
  redirect('/delivery/post?success=verify')
}
