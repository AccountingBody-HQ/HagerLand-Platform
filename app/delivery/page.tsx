...
export const metadata = {
  title: 'Delivery & courier services | HagerLand',
  description: 'Find a courier, freight, cargo, or removals service — or list your delivery business.',
}

const PAGE_SIZE = 20

export default async function DeliveryPage({ searchParams }: { searchParams: { type?: string; page?: string } }) {
  ...
  if (listingType) query = query.eq('category', listingType)
  const { data: listings, count, error } = await query

  const { data: typeRows } = await supabase.from('delivery').select('category')
    .eq('status', 'active').not('category', 'is', null)
  const listingTypes = Array.from(new Set((typeRows ?? []).map((r) => r.category).filter(Boolean) as string[])).sort()
      ...
      <section className="relative overflow-hidden bg-green">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div className="max-w-3xl">
            <p className="inline-flex items-center gap-2.5 text-white/50 text-[11px] font-bold tracking-[0.18em] uppercase mb-8">ሃገር <span className="w-1 h-1 rounded-full bg-white/30" translate="no" /> Homeland <span className="w-1 h-1 rounded-full bg-white/30" translate="no" /> Delivery & courier</p>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.05] tracking-tight mb-6">Delivery & courier<br /><span className="text-white/60">services,</span><br /><span className="text-white/60">worldwide.</span></h1>
            <p className="text-white/65 text-lg sm:text-xl max-w-xl mb-10 leading-relaxed">Find a courier, freight, cargo, or removals service — or list your delivery business.</p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-xl">
              ...
              <Link href="/delivery/post" className="inline-flex items-center justify-center gap-2 bg-white/15 hover:bg-white/25 border border-white/30 text-white font-bold rounded-full px-6 py-3.5 text-sm transition-colors whitespace-nowrap">Post a listing →</Link>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-24">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-8">
            <div className="flex items-center gap-3">
              {listingTypes.length > 0 && <FilterDropdown options={listingTypes} value={listingType} basePath="/delivery" paramName="type" allLabel="All types" />}
              {listingType && <Link href="/delivery" className="text-xs font-semibold text-green hover:underline">Clear</Link>}
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {listings && listings.length > 0 ? listings.map((listing) => (
              <Link key={listing.id} href={`/delivery/${listing.id}`}
                ...
                <div className="flex items-start gap-4 p-5 pb-4">
                  <div className="w-11 h-11 rounded-xl bg-green-soft flex items-center justify-center shrink-0">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-green"><path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted mt-0.5 truncate">{[listing.category, listing.city].filter(Boolean).join(' · ') || 'Community listing'}</p>
                  </div>
                ...
                <div className="flex items-center gap-4 px-5 py-3 border-t border-border bg-white mt-auto">
                  {listing.city && <span className="flex items-center gap-1.5 text-xs text-muted"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>{listing.city}</span>}
